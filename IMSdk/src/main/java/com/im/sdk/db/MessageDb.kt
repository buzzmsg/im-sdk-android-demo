...
class MessageDb private constructor(){
    ...
    fun updateStatus(message: MessageModel, status: Int, sendEvent: Boolean = true) {
        ...
        //send event
        if (message.type != com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt) {
...
//    ): MutableList<MessageModel>?
    ...
    @Query("SELECT * FROM tmm_message where id < :lastId and chatId = :chatId and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ORDER BY id Desc LIMIT :count")
    ...
    @Query("SELECT * FROM tmm_message where chatId == :chatId and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ORDER BY sequence DESC LIMIT 1")
    ...
    @Query("update tmm_message set status = :status Where mid = :mid and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt}")
    fun updateStatus(mid: String, status: Int)

    @Query("update tmm_message set status = :status Where mid in (:mids) and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt}")
    fun updateMessageStatusByMids(status: Int, mids: MutableSet<String>?)

    @Query("update tmm_message set crateTime = :createTime Where mid = :mid and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt}")
    fun updateCreateTime(mid: String, createTime: Long)

    @Query("update tmm_message set displayTime = :displayTime Where mid = :mid and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt}")
    ...
    @Query("SELECT * from tmm_message where status = :status and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ")
    fun queryMessagesByStatus(status: Int): MutableList<MessageModel>

    @Query("SELECT * FROM tmm_message where mid in (:mids) and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ")
    fun queryMessagesByMidsIncludeDel(mids: MutableList<String>?): MutableList<MessageModel>

    @Query("SELECT * FROM tmm_message where mid in (:mids) and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ")
    fun queryMessagesByMids(mids: MutableSet<String>?): MutableList<MessageModel>

    @Query("SELECT * FROM tmm_message where mid = :mid and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ")
    fun queryMessagesByMid(mid: String?): MessageModel?

    @Query("SELECT id FROM tmm_message where mid = :mid and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ")
    fun queryMessageIndexByMid(mid: String?): Long?

    @Query("SELECT * FROM tmm_message where chatId == :chatId and sender == :uid and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ")
    fun queryMyMessagesByChatId(chatId: String?, uid: String?): MutableList<MessageModel>

    @Query("SELECT * FROM tmm_message where chatId == :chatId and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ")
    ...
    @Query("SELECT * from tmm_message where id <:lastId and chatId = :chatId and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt}  ORDER BY id desc limit 1 ")
    fun queryMessageLimit(lastId: Long?, chatId: String?): MessageModel?

    @Query("SELECT * from tmm_message where chatId in (:chatIds) and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt}  ORDER BY id desc limit 1 ")
    fun queryMessagesLimit(chatIds: MutableSet<String>?): MutableList<MessageModel>?

    @Query("SELECT * from tmm_message where readStatus = ${MessageReadStatus.NOT_READ} and chatId = :chatId and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ")
    fun queryUnReadConversationMessagesByChatId(chatId: String?): MutableList<MessageModel>?

    @Query("SELECT * from tmm_message where readStatus = ${MessageReadStatus.NOT_READ} and chatId in (:chatIds) and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} ")
...
//    fun queryAtMessagesByChatId(chatId: String?, atTypes: MutableList<Int>): Int?
    ...
    @Query("update tmm_message set readStatus = :readStatus Where chatId = :chatId and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt}")
    ...
    @Query("update tmm_message set readStatus = ${MessageReadStatus.IS_READ} Where mid in (:mids) and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt}")
    ...
    @Query("SELECT * from tmm_message where chatId in (:chatIds) and delStatus != ${MessageDeleteStatus.IS_DEL}  and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} group by chatId having id = max(id)")
    fun queryMessageIndexByChatIds(chatIds: MutableSet<String>?): MutableList<MessageModel>?

    @Query("SELECT * from tmm_message where chatId in (:chatIds) and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt} group by chatId having sequence = max(sequence)")
    ...
    @Query("SELECT mid from tmm_message where mid not in (:mids) and chatId = :chatId and delStatus != ${MessageDeleteStatus.IS_DEL} and type != ${com.im.sdk.constant.MessageContentType.ContentType_Read_Receipt}  ORDER BY id desc limit 1 ")
